...
CI/CD Flow:
  Git Push → CI (ESLint → Jest/Coverage → npm audit → Trivy → Hadolint → ACR push)
           → CD (k8s validate → staging deploy → [approval gate] → production deploy)
```
...
|-------|------|---------------|
| **ESLint** | eslint | Zero linting errors |
...
```
CI passes → Build image tag → Push to ACR
...
| **Production gate** | GitHub Actions environment protection requires manual approval |
...
8. **Secrets:** Azure Key Vault CSI Driver configured; mount secrets as pod volumes without storing them in etcd.
9. **TLS:** cert-manager with Let's Encrypt for automated certificate management (ingress annotation-driven).
10. **Ingress:** NGINX Ingress Controller assumed to be installed on the cluster.
